...
>
> 日期：2026-08-15 · 版本：v4.21（DAP memory/disassembly 客户端闭环）· 状态：**实施中**。已有 M0–M11 代码保留，所有“已交付”结论仍须通过三端真机工程验收；本轮延续 IntelliJ IDEA 2026.2 与当前 DAP 规范基线，在 v4.20 instruction breakpoint 之上补齐 capability-gated 的 `readMemory`/`writeMemory`/`disassemble` 请求、bounded response parsing、hex/base64 转换、会话切换/终止 stale-response guard 与 DebugPanel 工具区。协议代码闭环不等于 IDEA 的真实调试器语义：真实 adapter 对 memory reference、地址宽度、读写权限、partial write、符号解析、指令字节和 source mapping 的解释，项目模型、PSI/index、原生 inspection/data-flow 与三端真实证据仍缺，因此**尚未达到 IntelliJ IDEA Code Editor 严格持平**。
>
...
> 更新于 2026-08-15（v4.21）；IntelliJ IDEA 2026.2 真实能力、DAP `exceptionOptions`/`breakpointModes`、data breakpoint address/range discovery、LSP 字段声明入口、instruction breakpoint 客户端生命周期、memory read/write/disassembly 客户端闭环、conditional exception filters、结构化 JUnit 测试结果、Compound Debug、function/method breakpoint、provider semantic snapshot、Inspection suppression/baseline/evidence 与自动化门禁已复核。M0–M5 已由 PR #361 合入 `main`；当前收口位于 `feat/code-workspace-idea-parity`。功能按代码与自动化门禁复核；Linux/macOS/Windows 真机冒烟和真实 adapter 支持矩阵由用户执行并待回填，不能据此宣称严格持平。完成度按本节拆分条目计数，新增代码闭环项在横切事项记录。
