...
>
> 日期：2026-08-15 · 版本：v4.22（DAP adapter contract fixtures）· 状态：**实施中**。已有 M0–M11 代码保留，所有“已交付”结论仍须通过三端真机工程验收；本轮延续 IntelliJ IDEA 2026.2 与当前 DAP 规范基线，在 v4.21 memory/disassembly 客户端闭环之上建立 Java/JavaScript/Python/Go/Rust/C++ 六项 adapter contract fixture，固定 opaque reference、signed byte offset、读写权限门控、显式 `allowPartial`、symbol/source mapping 与 mode applicability 的协议边界。fixture 的 advertised profile 是合成测试输入，不是某个 adapter 版本的能力声明；真实 adapter trace、项目模型、PSI/index、原生 inspection/data-flow 与三端真实证据仍缺，因此**尚未达到 IntelliJ IDEA Code Editor 严格持平**。
>
