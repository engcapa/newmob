...
>
> 日期：2026-08-15 · 版本：v4.18（DAP `breakpointModes` 真实能力收口）· 状态：**实施中**。已有 M0–M11 代码保留，所有“已交付”结论仍须通过三端真机工程验收；本轮延续 IntelliJ IDEA 2026.2 与当前 DAP 规范基线，在 v4.17 `exceptionOptions` 上补齐 adapter-advertised `breakpointModes`：source breakpoint 按 adapter 持久化 mode、exception filter 通过 capability-gated `filterOptions.mode` 发送、data breakpoint 在 `dataBreakpointInfo.mode` discovery 发送，并在 Debug UI 提供 capability-gated 选择和默认 mode。协议代码闭环不等于 IDEA 的硬件/软件断点、字段 watchpoint 或异常属性语义已在真实 adapter 上等价，项目模型、PSI/index、原生 inspection/data-flow 与三端真实证据仍缺，因此**尚未达到 IntelliJ IDEA Code Editor 严格持平**。
>
...
2. 标准协议支持和 IDEA 行为持平分开记账：例如 v4.17 `exceptionOptions` 与 v4.18 `breakpointModes` 只标记“DAP 代码闭环”，只有真实 adapter 的类继承、caught/uncaught、路径、硬件/软件 mode 和错误行为通过矩阵后，才能升级对应 IDEA 能力。
