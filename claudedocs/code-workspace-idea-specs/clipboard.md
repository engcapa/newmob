...
- **Audit:** `ready` as of the 2026-08-31 contract decision below. `createNativeClipboardPermissionAdapter()` still maps any non-null capability probe to `granted`. `workspaceClipboardSession.ts` still types the effect as the numeric `systemEffect: 0 | 1`, and three of its returns are factually wrong: after `await writeFn(text)` succeeds, a changed `permissionGeneration` returns `stale-generation` with `systemEffect: 0`, a permission that flipped to `denied` returns `denied` with `systemEffect: 0`, and a throw from inside the write returns `unavailable` with `systemEffect: 0` even though the OS write may already have landed.
...
- **Contract (approved 2026-08-31):** the numeric `systemEffect: 0 | 1` is replaced by the closed enum `"not-performed" | "performed" | "unknown"` on both the guarded write and read results. The mapping is fixed: `not-performed` only when the OS call was never entered, which is the pre-await denied path; `performed` once the OS call returned successfully, and it stays `performed` no matter what happens to the permission generation or permission state afterwards, because an external write is not revocable by later local knowledge; `unknown` whenever the call was entered but its completion cannot be established, which includes a throw from inside the write and any adapter that cannot confirm the result. Ownership and effect are independent axes: `outcome: "stale-generation"` with `systemEffect: "performed"` is the correct and expected combination, and the lease/ownership outcome must never be used to downgrade the effect. A `denied` outcome discovered only after a completed write likewise reports `performed`. `unknown` is never collapsed into either neighbour, and no path may report `not-performed` after entering the OS call.
...
- **Required evidence:** `code-audit`, `unit`, `typecheck`.
- **Decision record:** the effect-state contract above was requested on this card and approved by the maintainer on 2026-08-31, which is what moves the card from `review_required` to `ready`. The card is now claimable behind ED-CLIP-001; implementing the enum, migrating every `systemEffect` producer and consumer, and proving the three states is this card's own work, not part of the decision.
