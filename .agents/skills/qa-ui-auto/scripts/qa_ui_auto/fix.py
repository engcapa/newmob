...
def _fix_features_range(since: str | None) -> int:
    ...
    out.append("")
    out.append("Then update feature-list.md following the gen-from-range playbook:")
    out.append("  - Orphan NEW   → new feature OR extend an existing feature's `files`")
    out.append("  - Touched      → refresh description if capability changed")
    out.append("  - Deleted      → mark with HTML comment, decide manually")
    out.append("")
