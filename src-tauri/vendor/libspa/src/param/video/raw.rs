...
impl VideoInfoRaw {
    pub fn new() -> Self {
        // libspa-sys generates this structure from the installed headers.
        // PipeWire <= 0.3.48 has no `flags` field and uses an i64 modifier,
        // while newer headers add `flags` and change modifier to u64. All
        // fields are integer-backed C values, so zero is valid for both ABIs.
    ...
    pub fn set_flags(&mut self, flags: VideoFlags) {
        #[cfg(libspa_video_info_raw_has_flags)]
        {
            self.0.flags = flags.bits();
        }
        #[cfg(not(libspa_video_info_raw_has_flags))]
        {
            let _ = flags;
        }
    }
...
mod tests {
    ...
    #[test]
    fn video_info_raw_adapts_to_installed_spa_abi() {
        let mut info = VideoInfoRaw::new();
        assert_eq!(info.format(), VideoFormat::Unknown);
        assert_eq!(info.modifier(), 0);
        assert!(info.flags().is_empty());

        info.set_modifier(u64::MAX);
        assert_eq!(info.modifier(), u64::MAX);

        info.set_flags(VideoFlags::VARIABLE_FPS);
        #[cfg(libspa_video_info_raw_has_flags)]
        assert_eq!(info.flags(), VideoFlags::VARIABLE_FPS);
        #[cfg(not(libspa_video_info_raw_has_flags))]
        assert!(info.flags().is_empty());
    }
