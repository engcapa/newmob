        ...
        loop {
            ...
            tokio::select! {
                ...
                Ok((stream, peer)) = listener.accept() => {
                    // Input responses and small dirty rectangles must not wait
                    // for a later packet before they reach an interactive RDP
                    // client. In particular, terminal output is often only a
                    // few bytes after bitmap compression.
                    if let Err(error) = stream.set_nodelay(true) {
                        warn!(?peer, ?error, "Could not enable TCP_NODELAY for RDP client");
                    }
