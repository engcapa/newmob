...
vi.mock("../lib/ipc", () => ({
  ...
  listCommonLocalDirectoriesWithRevision: vi.fn(async () => ({ revision: 0, directories: [] })),
  listenWelcomeDirectoriesChanged: vi.fn(async () => () => undefined),
  recordLocalDirectoryUse: vi.fn(async () => ({ changed: false, directory: null })),
  listLocalShells: vi.fn(async () => []),
  listWslDistros: vi.fn(async () => []),
  markSessionConnected: vi.fn(async () => 0),
  writeTerminal: vi.fn(async () => undefined),
  getWelcomeRunSnapshot: vi.fn(async () => ({ snapshot: null, issue: null })),
  recordWelcomeRunSnapshot: vi.fn(async () => ({ snapshot: null, applied: false })),
  updateWelcomeRunContext: vi.fn(async () => ({ snapshot: null, applied: false })),
  clearWelcomeRunSnapshot: vi.fn(async () => true),
