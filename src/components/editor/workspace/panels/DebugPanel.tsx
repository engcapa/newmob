...
function parseSignedMemoryOffset(value: string): number | undefined | null {
...
function parsePositiveMemoryCount(value: string, max: number): number | null {
  const raw = value.trim();
  if (!/^\d+$/.test(raw)) return null;
  const parsed = Number(raw);
...
function MemoryDisassemblyView({ debug }: { debug: CodeDebugSession }) {
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        {state && (
          <>
            ...
            <Section title="Memory / Disassembly" defaultOpen={false}>
